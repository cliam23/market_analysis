...
On the Vercel-only deploy, `GET /api/dashboard/summary`, `GET /api/market/indices`, `GET /api/paper-trade/portfolio`, `GET /api/paper-trade/history`, `GET /api/paper-trade/preview`, and `GET /api/rl/status` (all `?universe=` where applicable) are also served — by `api/dashboard/summary.js`, `api/market/indices.js`, `api/paper-trade/portfolio.js`, `api/paper-trade/history.js`, `api/paper-trade/preview.js`, and `api/rl/status.js` respectively — as a **read-only mirror** of the same routes above, replaying whatever `scripts/generate-scores-snapshot.mjs` last captured (`publicMirror: true`, `mirroredAt` in the response). Every other route in this document, and all non-GET methods on these, require `server.js` running (locally, Railway, Docker, …).
