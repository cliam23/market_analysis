...
`GET /api/analysis/:ticker` is also mirrored (`api/analysis/[ticker].js`), for every ticker in `sp500_top150` (a superset of `sp500_top50` — see `scripts/lib/analysis-mirror.mjs`). Search works for real on any of those ~150 tickers; a ticker outside that list gets a 404 naming it explicitly.

`GET /api/rl/compare` (used by AlphaLabTab/RLTab with user-chosen periods), `GET /api/dcf/:ticker` and `GET /api/comps/:ticker` (per-model live compute, not a fixed matrix), `GET /api/diagnostics/*`, and the `/api/options/*` and `/api/wheel/*` routes (live chains, mutable paper positions) are **not** mirrored — `src/hooks/useBackendMode.js` detects the deploy at runtime and those tabs/sections/buttons show a clear "not available on this read-only deploy" message (or a disabled button with a tooltip, for actions) instead of attempting a doomed fetch.
